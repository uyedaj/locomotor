...
prior.11 <- make.prior(tree, plot.prior = FALSE, 
                       ...
                       param=list(dalpha=list(scale=0.1), dsig2=list(scale=0.1),
                                  dbeta_LnMass=list(mean=0.1, sd=1),
...
mcmc.NN <- bayou.makeMCMC(tree, dat, pred=pred, SE=MEvar, model=model.NN$model, prior=prior.NN, samp = 100, startpar=startpar.NN, new.dir="../output/LnMass/", outname="modelNN_r001", plot.freq=NULL)
mcmc.NN$run(gens)
